...
namespace Utils
{
    ...
    void checkSleepByLowBatteryVoltage(uint8_t mode)
    {
        if (shouldSleepLowVoltage)
        {
            ...
            if (mode == 1)
            { // low voltage detected after a while
                displayToggle(false);
            }
#ifdef VEXT_CTRL
    ...
    bool checkValidCallsign(const String &callsign)
    {
        ...
        { // SSID Validation
            ...
            if (ssid.indexOf("-") != -1 || ssid.length() > 2)
                return false;
            for (int i = 0; i < ssid.length(); i++)
            {
                if (!isAlphaNumeric(ssid[i]))
                    return false;
            }
        }
        else
        ...
        { // __0A__ must be validated
            if (cleanCallsign[0] != 'R' && !isDigit(cleanCallsign[1]) && !isAlpha(cleanCallsign[2]))
                return false; // to accepto R0A___
        }
